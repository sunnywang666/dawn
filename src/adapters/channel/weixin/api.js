...
async function sendText({ baseUrl, token, toUserId, text, contextToken, clientId, refMsgId }) {
  if (!String(contextToken || "").trim()) {
    throw new Error("weixin sendText requires contextToken");
  }
  const itemList = [];
  if (String(text || "").trim()) {
    const item = { type: 1, text_item: { text: String(text) } };
    const normalizedRefMsgId = String(refMsgId || "").trim();
    if (normalizedRefMsgId) {
      item.ref_msg = { message_id: normalizedRefMsgId };
    }
    itemList.push(item);
  }
  if (!itemList.length) {
