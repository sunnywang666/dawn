...
function assembleRuntimeTurnText({ prepared, config = {}, visionContext = {}, notionContext = null }) {
  const lines = [];
  ...
  const operationalGuidance = buildOperationalTurnGuidance({ originalText, config });
  if (operationalGuidance.length) {
    pushSectionBreak(lines);
    lines.push("Operational focus for this turn:");
    for (const item of operationalGuidance) {
      lines.push(`- ${item}`);
    }
  }

  if (notionContext?.summaryLines?.length) {
    pushSectionBreak(lines);
    lines.push("Current assignment context:");
    for (const item of notionContext.summaryLines) {
      lines.push(`- ${item}`);
    }
  }
...
function buildOperationalTurnGuidance({ originalText = "", config = {} } = {}) {
  ...
  const guidance = [];
  const notionReady = Boolean(normalizeText(config.notionApiKey) && normalizeText(config.notionAssignmentsDatabaseId));
  const isStudyOrScheduleTurn = /(作业|题目|课程|考试|复习|论文|截止|ddl|due|notion|任务|todo|待办|计划|安排|日程|schedule|上课)/iu.test(normalizedText);
  const isAvoidanceTurn = /(不想|不想写|拖延|烦|不想动|不想学|学不动|写不动|不想做)/iu.test(normalizedText);
  const wantsAgency = /(管着我|安排我|推进|推动|催我|盯着我|智能|聪明|搜相关|搜索相关|查一下|查查|notion)/iu.test(normalizedText);

  if (!isStudyOrScheduleTurn && !wantsAgency) {
    return guidance;
  }

  guidance.push("This user wants concrete help, not generic emotional filler.");
  guidance.push("Respond with forward motion: identify one clear next step, not just comfort or paraphrase.");
  ...
  if (isAvoidanceTurn) {
    guidance.push("If the user sounds avoidant or overwhelmed, do not keep pushing blindly. Narrow the work into the smallest concrete action that can start now.");
    guidance.push("Offer a bounded option such as opening the file, reading the prompt, or spending 10-15 minutes on one subtask. If appropriate, create a reminder for the next checkpoint.");
  }

  if (wantsAgency) {
    guidance.push("Act like an operator: search context first, choose the next lever, and help structure time or follow-up instead of chatting loosely.");
  }

  guidance.push("If there is enough context to schedule a follow-up, use the reminder tool directly instead of only promising to remember.");
  return guidance;
}

function shouldPrefetchNotionAssignments({ originalText = "", config = {} } = {}) {
  if (!normalizeText(config.notionApiKey) || !normalizeText(config.notionAssignmentsDatabaseId)) {
    return false;
  }
  const normalizedText = normalizeText(originalText);
  if (!normalizedText) {
  ...
  return /(作业|题目|课程|考试|复习|论文|截止|ddl|due|notion|任务|todo|待办|计划|安排|日程|schedule|上课|先做什么|最急|最先|催我|推进|推动|管着我)/iu.test(normalizedText);
}
