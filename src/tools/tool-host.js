...
const PROJECT_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        status: { type: "string", description: "open, all, Todo, Learning, In Progress, Completed, or Skip. Defaults to open." },
        limit: { type: "integer", description: "Maximum rows to return, 1-25." },
        query: { type: "string", description: "Optional title keyword to search." },
      },
      additionalProperties: false,
    },
    async handler({ services, args }) {
      const result = await services.notionTasks.listAssignments(args);
      return {
        text: `Notion assignments loaded: ${result.assignments.length}.`,
        data: result,
      };
    },
  },
  {
    ...
    inputSchema: {
      ...
      required: ["title"],
      properties: {
        title: { type: "string", description: "Assignment/exam title." },
        type: { type: "string", description: "Assignment or Exam. Defaults to Assignment." },
        status: { type: "string", description: "Todo, Learning, In Progress, Completed, or Skip. Defaults to Todo." },
        priority: { type: "string", description: "High, Medium, or Low. Defaults to Medium." },
        dueDate: { type: "string", description: "ISO date/datetime for Due Date, e.g. 2026-05-21 or 2026-05-21T23:59:00+08:00." },
        estimatedHours: { type: "number", description: "Estimated Time（h）." },
        requirement: { type: "string", description: "Clean assignment requirements extracted from the source." },
        subject: { type: "string", description: "Course/subject name. The tool will try to relate it to an existing Subject page." },
        subjectPageId: { type: "string", description: "Optional explicit Notion Subject page id when known." },
        sourceSummary: { type: "string", description: "Short summary of the chat/photo/file that produced this task." },
        sourceText: { type: "string", description: "Relevant original text excerpt from the forwarded chat, OCR, caption, or file." },
        sourceFilePaths: { type: "array", items: { type: "string" }, description: "Optional local inbox file paths used as evidence." },
        uncertain: { type: "array", items: { type: "string" }, description: "Fields or facts that need Bai Zhou's confirmation." },
      },
      additionalProperties: false,
    },
    async handler({ services, args }) {
      const result = await services.notionTasks.createAssignment(args);
      return {
        text: `Notion assignment created: ${result.title}`,
        data: result,
      };
    },
  },
