...
// Uses two methods so the check works regardless of whether CYBERBOSS_STATE_DIR is set
...
if (process.argv[2] === "start") {
  ...
  for (const stateDir of stateDirCandidates) {
    const pidFile =
      process.platform === "win32"
        ? path.join(stateDir, "windows-cyberboss-bridge.pid")
  ...
  if (!alreadyRunning && process.platform === "win32") {
    try {
      ...
      const lines = out.split(/\r?\n/).filter((l) => l.includes("cyberboss.js") && l.includes("start"));
  ...
  if (alreadyRunning) {
    console.log("[cyberboss] bridge already running, exiting.");
    process.exit(0);
  }
}
