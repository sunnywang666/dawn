...
# 《霸道总裁爱上患有 ADHD 的我》
## 支持 Codex 与 Claude Code 的微信桥接系统：Dawn
...
<p align="center">
  <img src="./docs/images/chat-example-1.jpg" alt="Dawn 示例对话 1" width="23%" />
  <img src="./docs/images/chat-example-2.jpg" alt="Dawn 示例对话 2" width="23%" />
  <img src="./docs/images/chat-example-3.jpg" alt="Dawn 示例对话 3" width="23%" />
  <img src="./docs/images/chat-example-4.jpg" alt="Dawn 示例对话 4" width="23%" />
</p>

Dawn 不是另一个平庸的番茄钟，也不是一个只会堆积任务的待办清单。
...
如果你对 `Dawn` 里最感兴趣的是“生活轨迹自动化报表”这一层，那么也可以直接把时间轴能力单独拿出去用：
...
`Dawn` 的时间轴能力本质上也是构建在 `timeline-for-agent` 之上，只是这里额外把它接进了微信、提醒、日记和随机轮询这整套生活监管链路里。
...
Dawn 假设你是一个完全不可控的个体：你不需要先点开始，不需要先记得回来，甚至不需要先拥有执行意志。你只需要继续活着、继续聊天，剩下的由系统去记录时间、补齐轨迹、主动出现。
...
DAWN_ALLOWED_USER_IDS=你的微信 user id
DAWN_WORKSPACE_ROOT=/绝对路径/你的项目目录
```
...
`DAWN_ALLOWED_USER_IDS` 支持逗号分隔多个 user id。

原因有两个：

- 第一次运行任意 `cyberboss` 命令时，会自动生成 `~/.exclusive-dawn/weixin-instructions.md`
- 如果你没先设置 `DAWN_USER_NAME` 和 `DAWN_USER_GENDER`，生成出来的 instructions 可能不符合真实情况

另外，如果你想要更强的“push 感”，建议一开始先不要主动大改 instructions 模板。先让 agent 在真实交流里自己更新行为，再回头只修明显不对的部分。

如果你要跑共享线程，建议也在第一次启动前就把 `DAWN_WORKSPACE_ROOT` 配好。这样 `shared:open` 会优先接到你当前项目对应的那条线程，而不是回退到别的历史绑定。

如果你使用 Ollama 这类本地 Codex provider，推荐用一个很小的 wrapper script，不要直接把 provider flags 塞进 `DAWN_CODEX_COMMAND`。把 [templates/codex-local-provider.sh](./templates/codex-local-provider.sh) 复制到 `${HOME}/.exclusive-dawn/codex-local`，给它执行权限，并让 Dawn 使用这个 wrapper：
...
```dotenv
DAWN_CODEX_COMMAND=/绝对路径/.exclusive-dawn/codex-local
DAWN_CODEX_MODEL_PROVIDER=ollama
DAWN_CODEX_MODEL=gemma4:26b-32k
```

这个模板会把云端和本地启动逻辑收敛在同一个 command 里。切回云端 provider 时，清空 `DAWN_CODEX_MODEL_PROVIDER` 和 `DAWN_CODEX_MODEL`，然后重启共享桥接，让 Codex app-server 用新的 command 环境启动。

本地 Codex 模型还需要 model metadata。如果 `DAWN_CODEX_MODEL` 指向的模型不在 Codex 内置 catalog 里，需要在 Codex home 里放一份模型 catalog，并在 `~/.codex/config.toml` 里引用：
...
当 `DAWN_RUNTIME=claudecode` 时，Dawn 会在当前工作区自动补写 `.mcp.json` 里的 `dawn_tools`，并在启动 Claude 时显式挂上这份 MCP 配置。Claude 能发现 Dawn project tools，靠的就是这条项目本地配置，而不是全局注册。
...
切换 runtime 只需要改 `DAWN_RUNTIME`。不需要为 Claude Code 单独学习另一套命令。
...
- 如果 `DAWN_RUNTIME=claudecode`，本地 Claude 窗口当前更适合作为共享线程的监听窗口
- 不要单独执行 `node ./bin/exclusive-dawn.js start --checkin`，除非已经明确设置 `DAWN_CODEX_ENDPOINT=ws://127.0.0.1:8765`
...
- Dawn 已内置 `whereabouts-mcp`，可以直接接收手机上传的定位、电量和触发原因。
- 如果要启用内置行踪服务，至少需要配置：
...
- 行踪数据默认写入 `~/.exclusive-dawn/locations.json`，不是写进项目目录。
...
- 标签表固定放在 `~/.exclusive-dawn/stickers/tags.json`。AI 会从这里读取可用标签，用户也可以直接手动增删改。
- 为了方便管理，当前表情包检索只做标签过滤，不做向量库召回。

<a id="agent-guide"></a>
## Agent 接入

给 agent 暴露的 Dawn 能力是项目内结构化工具。
...
- diary、reminder、timeline、screenshot、file-send 这类 Dawn 能力使用项目工具
...
因为第一次运行任意 `cyberboss` 命令时，会自动生成 `~/.exclusive-dawn/weixin-instructions.md`。先配好 `DAWN_USER_NAME` 和 `DAWN_USER_GENDER`，可以避免生成明显不符合现实的 instructions。
